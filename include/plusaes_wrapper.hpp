    ...
    class plusaes_wrapper {
        ...
        public:
            ...
            std::string cbc_encrypt(std::string raw, std::string pass) {
                ...
                const unsigned char* passkey = reinterpret_cast<unsigned char*>(pass.data());

                std::vector<unsigned char> e_raw_data(raw_sz);

                auto res = plusaes::encrypt_cbc(reinterpret_cast<unsigned char*>(raw.data()), orig_raw_sz, passkey, pass_sz, &m_iv, &e_raw_data[0], e_raw_data.size(), true);
                if (res) {
                    return "";
                }
            ...
            std::string cbc_decrypt(std::string raw, std::string pass) {
                ...
                auto res = plusaes::encrypt_ecb(reinterpret_cast<unsigned char*>(raw.data()), orig_raw_sz, passkey, pass_sz, &e_raw_data[0], e_raw_data.size(), true);
                if (res) {
                    return "";
                }
            ...
            std::string gcm_encrypt(std::string raw, std::string pass, std::string tag, std::string aad = "") {
                const unsigned long pass_sz = pass.size(), orig_raw_sz = raw.size();
                if (orig_raw_sz == 0 || (pass_sz != 16 && pass_sz != 24 && pass_sz != 32)) return "";
                ...
                const unsigned long raw_sz = plusaes::get_padded_encrypted_size(raw.size());
                const unsigned char* passkey = reinterpret_cast<unsigned char*>(pass.data());
                unsigned char* aadata = reinterpret_cast<unsigned char*>(aad.data());
                int aadata_sz = aad.size();

                //std::vector<unsigned char> vec_iv(std::begin(m_iv), std::end(m_iv));
                std::vector<unsigned char> e_raw_data(raw_sz);

                auto res = plusaes::encrypt_gcm(reinterpret_cast<unsigned char*>(raw.data()), orig_raw_sz, aadata, aadata_sz, passkey, pass_sz, &m_tw_iv, &m_tag);
                if (res) {
                    return "";
                }
