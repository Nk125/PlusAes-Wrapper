...
int main(int argc, char* argv[]) {
    std::string type, in, out, pass, con, aes_str, prefix;
    bool decrypt_mode;
    int pass_sz;
    ...
    if (argc <= 1) {
        std::cerr << "Usage: -d/-e [Input File] [Output File] [Password]\n";
        return 1;
    }
    if (argc <= 2) {
        ...
        try {
            con = b.read_file(in);
            //std::string iv = "nk125&pa_endelib";
            // NK125 and PlusAES Encrypt-Decrypt Library (for C++)
            // Only for use in ECB En-Decryption AES type
            //aes.set_iv(reinterpret_cast<unsigned char*>(iv.data()));

            if (!decrypt_mode) {
                con = aes.cbc_encrypt(con, pass);
            }
            else {
                con = aes.cbc_decrypt(con, pass);
            }
